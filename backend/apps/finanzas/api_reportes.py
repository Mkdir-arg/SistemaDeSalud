...
class ReporteFinanzasViewSet(ConsultaReporte):
    ...
    def evolucion(self, request):
        ...
        calendarios = [list(calendario_mensual(expectativas, request.user, mes).values(
            ...
            "gastos_pendientes", "gastos_aprobados", "importe_aprobado", "monto_referencia",
        )) for mes in periodos]
        ...
        for mes, calendario in zip(periodos, calendarios):
            ...
            for serie in series or [{"id": None, "meses": meses_vacios}]:
                ...
                estado = "sin_control" if not filas else "sin_carga" if falta and not registros else "incompleto" if falta or pendientes else "mes_abierto" if mes >= actual else "completo"
                serie["meses"].append({
    ...
    def list(self, request):
        ...
        ajustes = AjusteGasto.objects.filter(gasto_id=OuterRef("pk")).order_by().values("gasto_id").annotate(total=Sum("importe")).values("total")[:1]
