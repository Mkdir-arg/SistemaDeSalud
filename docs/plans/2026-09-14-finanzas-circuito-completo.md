...
- Frontend: `npx playwright test --config playwright.finanzas-ui.config.js` con `CAUCE_URL=http://127.0.0.1:5178`: 9/9 pruebas Chromium aprobadas en 19,1 s, con API simulada. `npm run build`: 149 módulos, correcto (1,22 s). Estas pruebas no demuestran integración con la base de demo ni ejecutan actividad clínica del usuario.
