...
## Ajuste aprobado posterior: estados y referencias de todo el período

El usuario entregó el feedback y pidió implementarlo en esta misma sesión: filtro de carga/aprobación completas, incompleto/abierto o ambos; referencias de todos los conceptos seleccionados que tengan al menos una referencia, sin selector individual. Confirmó expresamente que las referencias deben conservar **todo el período**, independientemente del filtro de estado.

Plan local: reutilizar Select y los datos ya consultados; aplicar estado por concepto/mes al gráfico, tooltip y listado, sin modificar importes ni referencias. Ambos por defecto. Sin carga sigue siendo ausencia, no un cero graficado; referencia cero sí es válida. Probar filtros cruzados entre categorías, referencias sin datos/parciales/cero, ausencia de solicitudes adicionales y regresiones de navegación y layout. Sin backend, migraciones ni nuevas dependencias. Se reutiliza este plan porque writing-plans no está disponible.

Resultado: implementado en ResumenFinanzas.jsx y GraficoFinanzas.jsx, con los predicados comunes de siete líneas en evolucion.js para mantener gráfico, tooltip y listado consistentes. La comparación de referencias queda deshabilitada si ningún concepto seleccionado tiene alguna; una referencia aislada se dibuja como punto pequeño. Si no hay importes aprobados graficables con el filtro, se informa sin retirar las referencias disponibles.

Validación final: **45 pruebas OK** con `node node_modules/@playwright/test/cli.js test --config=playwright.finanzas-ui.config.js`, 1,3 minutos. Nuevo caso cubre estado distinto entre conceptos del mismo mes, referencias de todo el período, cero aislado, ausencia parcial/total, coherencia de importes ocultos, selección y período, estado vacío con referencia conservada y ausencia de consultas por cambiar el filtro local. Capturas de escritorio/móvil inspeccionadas. Un selector inicial de la prueba buscaba puntos dentro del grupo SVG equivocado; se constató su existencia en la capa de puntos de Recharts y se ajustó la prueba sin parchear el gráfico para satisfacerla.

`npm run build`: OK, 738 módulos; `git diff --check`: OK; frontend 8090 responde HTTP 200. Sin cambios de backend ni base: no se repitieron sus tests, ni CI, Firefox/Safari o un recorrido autenticado de demo. La suite UI usa contratos HTTP simulados. Skills: brainstorming para registrar la decisión confirmada; interface-design para reutilizar controles/tokens; vercel-react-best-practices para datos derivados y filtrado local; systematic-debugging para la comprobación SVG; playwright para regresiones/capturas de la suite autorizada; handoff para actualizar la continuidad. La aceptación visual humana sigue pendiente.
