...
Las bases de pruebas se crean y retiran con el runner de Django; no se usa `cauce` como base de test. En el proceso de validación se fijaron `DATABASES['default']['NAME'] = 'finanzas_postdemo_validation'` y `DATABASES['default']['TEST']['NAME'] = 'test_finanzas_postdemo_final_20260915'` antes de invocar el runner; no se editaron ajustes persistentes. Los errores y respuestas 403/404/409/503 del log corresponden a pruebas negativas deliberadas. Los artefactos quedaron fuera del repositorio, en la carpeta temporal local.
