...
function ConfiguracionHospital({ opciones, institucion, actualizar }) {
  ...
  return <div className="space-y-4">{error && <ErrorPortal error={error} />}{mensaje && <p role="status" className="rounded-md bg-badge-green-bg p-3 text-badge-green-fg">{mensaje}</p>}
    <Card className="p-5"><h2 className="text-lg font-semibold">Circuito de cobertura</h2><form className="mt-4 max-w-xl space-y-4" onSubmit={(e) => guardar(e, "configurar", { institucion: institucion.id, activo, dias_reserva_antigua: Number(dias) })}><Checkbox label="Habilitar cobertura y distribución de cobros para este hospital" checked={activo} onChange={(e) => setActivo(e.target.checked)} disabled={ocupado} /><div className="w-full max-w-sm"><Field label="Considerar antigua una reserva después de (días)" hint="Sólo la señala para revisión. No libera cupos automáticamente."><Input className="w-full" type="number" min={1} max={365} value={dias} onChange={(e) => setDias(e.target.value)} required disabled={ocupado} /></Field></div><Button disabled={ocupado}>Guardar configuración</Button></form></Card>
...
function AtencionCobertura({ scope, institucion, opciones, actualizar }) {
  ...
  return <div className="space-y-4"><Card className="space-y-4 p-5"><div className="min-w-0"><Field label="Buscar paciente o caso"><Input value={numero} onChange={(e) => setNumero(e.target.value)} placeholder="Nombre, documento o número de caso" /></Field></div>{consulta.error && <ErrorPortal error={consulta.error} reintentar={consulta.refetch} />}{buscar && consulta.data && !consulta.data.casos?.length && <p className="text-sm text-texto-debil">No hay casos disponibles con esa búsqueda dentro de tus permisos.</p>}<Field label="Caso"><Select value={caso} onChange={(e) => setCaso(e.target.value)}><option value="">Seleccioná un caso</option>{candidatos.map((item) => <option key={item.id} value={item.id}>{item.paciente ? `${item.paciente} · ` : ""}{item.titulo || `Caso ${item.id}`}{item.documento ? ` · ${item.documento}` : ""}</option>)}</Select></Field><p className="text-sm text-texto-debil">La búsqueda se actualiza dos segundos después de escribir. La afiliación elegida se conserva para este caso aunque cambie el padrón.</p></Card>{seleccionado && <OperacionCaso key={caso} caso={seleccionado} scope={scope} institucion={institucion} opciones={opciones} actualizar={actualizar} />}</div>;
}
