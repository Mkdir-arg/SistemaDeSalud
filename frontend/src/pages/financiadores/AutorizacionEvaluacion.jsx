import { Ayuda, Badge } from "@/components/ui";
...
export default function AutorizacionEvaluacion({ evaluacion }) {
  ...
  return <div className="space-y-2 rounded-md border border-division p-3 text-sm" role="region" aria-label="Autorización de esta evaluación">
    <div className="flex flex-wrap items-center gap-2"><span className="flex items-center gap-2"><span className="font-semibold">Autorización previa</span><Ayuda>Los importes de esta evaluación todavía no son cuentas por cobrar.</Ayuda></span><Badge tone={aprobada ? "green" : "amber"}>{ESTADOS_EVALUACION_AUTORIZACION[evaluacion.estado_autorizacion] || "Pendiente de verificar"}</Badge></div>
