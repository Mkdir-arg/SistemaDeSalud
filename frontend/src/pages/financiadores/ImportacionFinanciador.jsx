...
import { Ayuda, Badge, Button, Card, Field, Input, Modal, Spinner, Textarea } from "@/components/ui";
...
export default function ImportacionFinanciador({ organizacion, tipo, scope, onClose, onAplicado }) {
  ...
  const lotes = useQuery({ queryKey: [...scope, "importaciones", tipo, pagina], queryFn: () => api.get(`${rutaFinanciador(organizacion.id, "importaciones")}?${new URLSearchParams({ tipo, page: pagina, page_size: POR_PAGINA })}`), gcTime: 0, refetchInterval: lote && EN_PROCESO.has(lote.estado) ? 2000 : false });
  useEffect(() => {
  ...
  return <Modal title={`Importar ${tipo === "padron" ? "padrón" : "consumos externos"}`} onClose={ocupado ? undefined : onClose} width={840}>
    <div className="space-y-5">
      ...
      <Card className="p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="flex items-center gap-2"><h3 className="font-semibold">1. Prepará el archivo</h3><Ayuda>Descargá la plantilla vigente y completá la hoja de carga. Conservá los identificadores como texto.</Ayuda></div></div><Button variant="secondary" disabled={descargando} onClick={() => descargar("plantilla", { tipo }, `plantilla-${tipo}.xlsx`)}>Descargar plantilla</Button></div></Card>
      ...
      {lote && <section aria-label="Resumen de importación" className="space-y-4 border-t border-division pt-5">
        ...
        {procesando ? <Spinner label="Procesando el lote. Podés volver a consultarlo desde las importaciones recientes." /> : !terminado && <><div className="flex items-center gap-2"><Button disabled={ocupado || (validas === 0 && !hayRevision && !resumen.error_tecnico)} onClick={confirmar}>{ocupado ? "Importando…" : resumen.error_tecnico ? "Reintentar confirmación" : hayRevision ? "Importar filas válidas y revisadas" : `Importar ${plural(validas, "fila válida", "filas válidas")}`}</Button><Ayuda>Las filas válidas se vuelven a verificar al confirmar. El resultado final indica cuáles se aplicaron; las rechazadas quedan disponibles para corregir.</Ayuda></div></>}
