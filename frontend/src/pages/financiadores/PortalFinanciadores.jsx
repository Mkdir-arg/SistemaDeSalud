...
function EspacioFinanciador({ organizacion, usuarioId, plataforma, tab }) {
  ...
  const puedeCrear = !["actividad", "aranceles"].includes(tab) && (["padron", "consumos"].includes(tab) ? operador : admin);
  ...
  return <>
    ...
    <section aria-label={SECCIONES.find((item) => item.key === tab)?.label}>
      ...
      <ListaPortal key={tab} recurso={tab} organizacion={organizacion} scope={scope} admin={admin} operador={operador} planes={planes.data || []} catalogo={catalogo.data || []} actualizado={actualizado} />
    </section>
...
function ListaPortal({ recurso, organizacion, scope, admin, operador, planes, catalogo, actualizado }) {
  ...
  if (recurso === "convenios" && admin) columnas.push({ key: "acciones", label: "Acciones", render: (r) => <div className="flex flex-wrap gap-2">{r.estado === "propuesto" && r.propuesto_por === "hospital" && <><Button size="sm" variant="secondary" disabled={aceptando != null} onClick={() => aceptar(r)}>{aceptando === r.id ? "Aceptando…" : "Aceptar convenio"}</Button><Button size="sm" variant="ghost" onClick={() => setVigencia({ accion: "rechazar-convenio", fila: r })}>Rechazar propuesta</Button></>}{r.estado === "activo" && <Button size="sm" variant="ghost" onClick={() => setVigencia({ accion: "cerrar-convenio", fila: r })}>Cerrar convenio</Button>}{["rechazado", "finalizado"].includes(r.estado) && "—"}</div> });
...
function columnasDe(recurso, planes, catalogo) {
  ...
  return {
    ...
    convenios: [{ key: "institucion_nombre", label: "Hospital" }, { key: "estado", label: "Estado", render: (r) => <Badge tone={r.estado === "activo" ? "green" : r.estado === "propuesto" ? "amber" : "gray"}>{ESTADOS_CONVENIO[r.estado] || r.estado}</Badge> }, { key: "propuesto_por", label: "Propuesto por", render: (r) => ({ hospital: "Hospital", financiador: "Financiador", plataforma: "Plataforma" })[r.propuesto_por] || r.propuesto_por }, { key: "aceptado_en", label: "Aceptado el", render: (r) => fechaHora(r.aceptado_en) }, { key: "cerrado_en", label: "Cierre / rechazo", render: (r) => r.cerrado_en ? <>{fechaHora(r.cerrado_en)}<p className="mt-1 text-xs text-texto-debil">{r.motivo_cierre}</p></> : "—" }],
    usuarios: [{ key: "email", label: "Correo electrónico" }, nombre, { key: "rol", label: "Rol", render: (r) => ROLES[r.rol] || r.rol }, { key: "activo", label: "Estado", render: (r) => <Badge tone={r.activo === false ? "gray" : "green"}>{r.activo === false ? "Inactivo" : "Activo"}</Badge> }],
    actividad: [{ key: "fecha", label: "Fecha", render: (r) => fecha(r.fecha) }, { key: "hospital", label: "Hospital" }, { key: "numero", label: "N.º de afiliado" }, { key: "documento", label: "Documento" }, { key: "prestacion", label: "Prestación" }, { key: "cantidad", label: "Cantidad" }, { key: "cubiertas", label: "Unidades cubiertas" }, { key: "estado", label: "Estado", render: (r) => ({ reservada: "Reservada", realizada: "Realizada", liberada: "Liberada" })[r.estado] || r.estado }, { key: "importe_financiador", label: "Importe del financiador", render: (r) => importeARS(r.importe_financiador) }, { key: "estado_cobro", label: "Distribución del cobro", render: (r) => ({ resuelta: "Responsable definido", pendiente: "Pendiente de resolución", arancel_pendiente: "Arancel pendiente", evaluacion_pendiente: "Evaluación pendiente", sin_cobro: "Sin cobro" })[r.estado_cobro] || "Al realizar la prestación" }, { key: "discrepancia", label: "Revisión", render: (r) => r.discrepancia ? <Badge tone="amber">Discrepancia</Badge> : "—" }, { key: "acceso", label: "Alcance del acceso", render: (r) => r.acceso === "pendiente_historico" ? <Badge tone="amber">Histórico pendiente</Badge> : r.acceso === "vigente" ? "Relación vigente" : "—" }],
  }[recurso];
}
