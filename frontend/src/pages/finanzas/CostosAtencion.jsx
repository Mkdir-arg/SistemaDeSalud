...
import { Ayuda, Badge, Button, Card, Checkbox, Field, Input, Modal, Select, Spinner, Textarea } from "@/components/ui";
...
export default function CostosAtencion({ institucion, permisos, mes, area, areas, onGasto }) {
  const tabla = useTablaUrl("costos", { tamanoInicial: POR_PAGINA });
...
function DetalleCosto({ fila, permisos, nombreArea, onGasto, onClose }) {
  ...
  return <Modal title={`Costo de atención · Caso #${fila.caso}`} onClose={onClose} width={780} footer={<Button variant="secondary" onClick={onClose}>Cerrar detalle</Button>}><div className="space-y-6">
    ...
    <section aria-label="Importes conocidos de la atención" className="grid gap-4 rounded-md bg-superficie-2 p-4 sm:grid-cols-2">
      <div><div className="flex items-center gap-1.5"><p className="text-sm text-texto-debil">Directo conocido</p><Ayuda>Incluye sólo ajustes aprobados de componentes.</Ayuda></div><p className="mt-1 break-all text-cifra font-semibold tabular-nums">{importeARS(fila.total_conocido)}</p></div>
      <div><div className="flex items-center gap-1.5"><p className="text-sm text-texto-debil">Compartido conocido</p><Ayuda>Parte de gastos aprobados atribuida aquí.</Ayuda></div><p className="mt-1 break-all text-cifra font-semibold tabular-nums">{fila.reparto_actualizando ? "Actualizando reparto" : importeARS(fila.total_compartido_conocido)}</p></div>
    </section>
...
export function ConfiguracionCostos({ institucion, permisos, onClose }) {
  ...
  return <Modal title="Configurar costos por atención" ayuda="Configurá qué se cuenta una vez al completar cada atención. No crea cargos ni pagos y no reescribe los costos históricos." onClose={form ? undefined : onClose} width={840}>
...
function FormularioCosto({ tipo, fila, institucion, permisos, prestacion, componente, atenciones, prestaciones, onClose, onGuardado }) {
  ...
  return <form onSubmit={guardar} className="space-y-4"><h3 className="text-lg font-semibold">{titulos[tipo]}</h3>
    <fieldset disabled={guardando || incierto} className="space-y-4">
      ...
      {tipo !== "valor" && <><Field label="Nombre"><Input required maxLength={160} value={datos.nombre} onChange={(e) => set("nombre", e.target.value)} /></Field><details className="border-t border-division pt-3"><summary className="cursor-pointer text-sm font-medium">Opciones avanzadas</summary><div className="mt-3"><Field label="Código de referencia (opcional)" ayuda="Si lo dejás vacío, el sistema genera un código interno. Completalo sólo si usás una referencia propia."><Input maxLength={60} value={datos.codigo} onChange={(e) => set("codigo", e.target.value)} /></Field></div></details></>}
