...
import { POR_PAGINA, query, useLista } from "@/api/queries";
import { Ayuda, Badge, Button, Card, Field, Input, Modal, Select, Spinner, Textarea } from "@/components/ui";
...
export default function DineroFinanzas({ institucion, permisos, mes, area }) {
  const tabla = useTablaUrl("dinero", { tamanoInicial: POR_PAGINA });
  ...
  return <div className="space-y-5">
    ...
    <DataTable adaptable filas={cuentas.error ? [] : cuentas.filas} total={cuentas.error ? 0 : cuentas.total} paginas={cuentas.paginas} tabla={tabla}
      mantenerEncabezados barra={<BusquedaTablaFinanzas filtros={busqueda} label="Buscar cuentas" placeholder="Nombre o referencia de la contraparte…" />}
...
function ResumenDinero({ institucion, permisos, area, onCuenta }) {
  ...
  return <Card className="space-y-4 p-4"><div className="flex flex-wrap items-end justify-between gap-3"><div><div className="flex items-center gap-2"><h2 className="text-lg font-semibold">Dinero registrado</h2><Ayuda>Los totales incluyen sólo registros aprobados, por fecha real del pago, cobro o devolución. Este mes es independiente del mes económico de las cuentas.</Ayuda></div></div><Field label="Mes de pagos y cobros"><Input type="month" required value={mesDinero} onChange={(e) => setMesDinero(e.target.value)} /></Field></div>
...
export function MovimientosPeriodo({ filtros, institucion, permisos, onCuenta }) {
  const tabla = useTablaUrl("movimientos_dinero", { tamanoInicial: POR_PAGINA });
  ...
  return <section className="space-y-3 border-t border-division pt-4"><DataTable adaptable mantenerEncabezados barra={<div className="flex items-center gap-2"><BusquedaTablaFinanzas filtros={busqueda} label="Buscar movimientos" placeholder="Contraparte o referencia del movimiento…" /><Ayuda>Cada pago, cobro o devolución del período, aunque su cuenta pertenezca a otro mes económico.</Ayuda></div>} filas={consulta.error ? [] : consulta.filas} total={consulta.error ? 0 : consulta.total} paginas={consulta.paginas} tabla={tabla} estado={{ cargando: consulta.isLoading, error: consulta.error, reintentar: consulta.refetch }} vacio={{ titulo: "Sin movimientos en estas fechas" }} columnas={[
...
export function DetalleCuenta({ id, institucion, permisos, onClose }) {
  ...
  return <Modal title={`Cuenta #${id}`} onClose={accion ? undefined : onClose} width={780}>
    ...
    {fila && (accion ? accion.tipo === "decision"
      ...
      {["por_aprobar", "reintegros_por_aprobar", "ajustes_por_aprobar"].some((campo) => importeValido(fila[campo])) ? <section aria-label="Reservas por aprobar" className="space-y-3 border-t border-division pt-3"><div className="flex items-center gap-2"><h4 className="text-sm font-semibold">Reservas por aprobar</h4><Ayuda>El saldo y el dinero neto anteriores incluyen sólo lo aprobado. Los registros pendientes reservan importes para evitar cargarlos dos veces.</Ayuda></div><dl className="grid gap-3 sm:grid-cols-3">{[["Pagos / cobros por aprobar", "por_aprobar"], ["Devoluciones por aprobar", "reintegros_por_aprobar"], ["Reducciones por aprobar", "ajustes_por_aprobar"]].map(([label, campo]) => <div key={campo}><dt className="text-sm text-texto-debil">{label}</dt><dd className="tabular-nums">{importeARS(fila[campo])}</dd></div>)}</dl></section> : <p className="border-t border-division pt-3 text-sm text-texto-debil">Sin reservas por aprobar.</p>}
...
function OperacionCuenta({ fila, tipo, movimiento, puedeAprobar, onClose, onGuardado }) {
  ...
  return <form onSubmit={devolver && !previewActual ? previsualizar : guardar} className="space-y-4"><h3 className="text-lg font-semibold">{titulo}</h3><p className="text-sm text-texto-debil">{fila.contraparte_nombre} · Máximo: {importeARS(maximo)}</p>
    ...
    <fieldset disabled={bloqueado} className="space-y-4"><Field label="Importe en ARS" ayuda={reducir ? "Esta operación reduce la obligación. No devuelve dinero. Para una devolución, usá el pago o cobro original de la cuenta." : undefined}><Input required inputMode="decimal" pattern="[0-9]+([.,][0-9]{1,2})?" value={importe} onChange={(e) => setImporte(e.target.value)} /></Field>
      ...
      {devolver && <><Field label="Qué pasa con la cuenta"><Select value={efecto} onChange={(e) => setEfecto(e.target.value)}><option value="mantener">Sigue pendiente: habrá que volver a pagar o cobrar</option><option value="reducir">Se reduce o cancela la cuenta por este importe</option><option value="reduccion_existente">La cuenta ya se redujo: vincular esa reducción</option></Select></Field>{efecto === "reduccion_existente" && <Field label="Reducción ya registrada" ayuda="Vincular una reducción existente evita descontar dos veces la misma cancelación."><Select required value={ajuste} onChange={(e) => setAjuste(e.target.value)}><option value="">Elegí la reducción</option>{(fila.ajustes || []).filter((a) => importeValido(a.disponible_reintegro)).map((a) => <option key={a.id} value={a.id}>#{a.id} · {a.motivo} · Disponible {importeARS(a.disponible_reintegro)}</option>)}</Select></Field>}<p className="text-sm text-texto-debil">La devolución se vincula al {fila.tipo === "pagar" ? "pago" : "cobro"} #{movimiento.id}.</p></>}
    </fieldset>
